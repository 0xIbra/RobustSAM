...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Limit to a maximum number of masks, e.g., 10
        max_allowed_masks = 50
        if all_mask.shape[0] > max_allowed_masks:
            all_mask = all_mask[:max_allowed_masks]
        elif all_mask.shape[0] < max_allowed_masks:
            padding = np.zeros((max_allowed_masks - all_mask.shape[0], *all_mask.shape[1:]), dtype=all_mask.dtype)
            all_mask = np.concatenate([all_mask, padding], axis=0)

        all_mask = torch.tensor(all_mask, dtype=torch.float32)  # Ensure tensor type
